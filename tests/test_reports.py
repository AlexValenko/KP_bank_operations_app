...
from src.reports import spending_by_category, spending_by_weekday, write_down_report_default
...
def test_spending_by_category_success(get_test_transactions) -> None:
    ...
    result_dict = spending_by_category(
        transactions=get_test_transactions, category="Медицина", date="01-06-2026"
    ).to_dict("records")
    assert len(result_dict) == 1
    assert result_dict[0]["Сумма платежа"] == -3400.0


def test_spending_by_category_non_category(get_test_transactions) -> None:
...
def test_decorator_write_down_report_default_success(get_test_transactions) -> None:
    """ПРимер успешной работы декоратора write_down_report_default
    на примере функции spending_by_weekday на тестовых данных"""
    spending_by_weekday(transactions=get_test_transactions, date="01-04-2026").to_dict("records")
    with open("data/default_reports.json", "r", encoding="utf-8") as file_report:
        report_data = json.load(file_report)
    assert report_data[0] == {"День недели": "Понедельник", "Сумма операции": -31660.1}


def test_decorator_write_down_report_select_file(get_test_transactions) -> None:
    spending_by_category(transactions=get_test_transactions, category="Медицина", date="01-06-2026").to_dict("records")

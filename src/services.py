...
def get_cashback_categories(data: pd.DataFrame, year:int, month:int) -> json:
    ...
    if filtered_by_category_df.empty:
        services_logger.warning(f'Не найдено подходящих транзакций для начисления кэшбэка')
        print("Не найдено подходящих транзакций для начисления кэшбэка")
        return None
    services_logger.debug(f'Транзакции отфильтрованы статусу и категории, выбрано {len(filtered_by_category_df)} записей')
    ...
    services_logger.debug(f'Транзакции сгруппированы по категориям, вычислен расчетный кэшбэк \n')
...
def investment_bank(month: str, transactions: pd.DataFrame, limit: int) -> float:
    ...
    if investment_bank_df.empty:
        services_logger.warning(f'Не найдено подходящих транзакций для вычисления округлений по операциям')

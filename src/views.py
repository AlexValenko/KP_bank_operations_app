...
def get_main_page_data(current_date: str = None, path_excel_file: str = "data/operations.xlsx") -> json:
    ...
    if current_date is None:
        ...
        try:
            ...
            views_logger.error(
                f"Ошибка {e} чтения даты из аргументов функции, требуется строка в формате YYYY-MM-DD HH:MM:SS"
            )
            raise ValueError("Введен некорректный формат даты")
    start_date_dt = current_date_dt.replace(day=1, hour=0, minute=0)
    views_logger.debug(
        f"Выбран временной диапазон с {start_date_dt.strftime(format='%d-%m-%Y')}"
        f" по {current_date_dt.strftime(format='%d-%m-%Y')}"
    )

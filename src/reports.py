...
reports_logger = logging.getLogger('services')
...
def write_down_report_default(func:callable) -> None:
    ...
    def wrapper(*args, **kwargs):
        reports_logger.debug(f'Вызван декоратор write_down_report_default')
...
def write_down_report_select_file(file_path):
    ...
    def wrapper (func: callable):
        def inner(*args, **kwargs):
            reports_logger.debug(f'Вызван декоратор write_down_report_select_file')
            result = func(*args, **kwargs)
            if not isinstance(result, pd.DataFrame):
                reports_logger.error('Декорируемая функция должна возвращать DataFrame')
                raise TypeError("Функция должна возвращать DataFrame")
...
def spending_by_category(transactions: pd.DataFrame,
    ...
    reports_logger.debug(f'Выбран диапазон дат с {start_date.strftime("%d-%m-%Y")}  по {end_date.strftime("%d-%m-%Y")}')
    ...
    if cleaned_for_calc_df.empty:
        reports_logger.warning(f'Не найдено подходящих транзакций')
        return pd.DataFrame()
...
def spending_by_weekday(transactions: pd.DataFrame,
    ...
    reports_logger.debug(f'Транзакции отфильтрованы по типу, статусу и валюте, выбрано {len(cleaned_for_calc_df)} записей')

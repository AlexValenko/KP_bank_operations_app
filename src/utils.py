...
# При импорте файла выполняется валидация данных на соответствие столбцов таблицы. В функциях обработки повторная валидация не выполняется.
REQUIRED_COLUMNS = ['Дата операции', 'Дата платежа', 'Номер карты', 'Статус', 'Сумма операции', 'Валюта операции', 'Кэшбэк', 'Категория', 'Описание', 'Округление на инвесткопилку']
